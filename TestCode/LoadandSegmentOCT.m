...
[sloBars_h,  mapSLO_h] = OCTBarsOnSLO(slo_h,header_h,BScanHeader_h,BScans_h,thic_h);
...
[header_v, BScanHeader_v, slo_v, BScans_v] = openVolFast(fn);
%[boundaries_v, img_vol_v] = quickSegmentOCT(fn);
thic_v = boundaries_v(:,:,bndCompareB)-boundaries_v(:,:,bndCompareT);

[sloBars_v,  mapSLO_v] = OCTBarsOnSLO(slo_v,header_v,BScanHeader_v,BScans_v,thic_v);
...
[sloBars_combined, ~] = OCTBarsOnSLO(sloBars_v_reg,header_h,BScanHeader_h,BScans_h,thic_h);
mapSLO_combined = (mapSLO_v_interp_reg + mapSLO_h_interp)/2;
mapSLO_diff = abs(mapSLO_v_interp_reg*header_v.ScaleZ*1000 - mapSLO_h_interp*header_h.ScaleZ*1000);
...
imagesc(mapSLO_diff);
colormap('default')
caxis([0  prctile(mapSLO_diff(:),99)])
colorbar
%end
